...
async def test_add_tweet(rollbacked_db_session, new_tweet):
    rollbacked_db_session.add(new_tweet)
    await rollbacked_db_session.commit()

    queryset = await rollbacked_db_session.execute(
...
async def test_content_length(rollbacked_db_session, new_tweet):
...
async def test_content_length_truncate(rollbacked_db_session, new_tweet):
    """Проверка обрезания по длине поля content."""
    rollbacked_db_session.add(new_tweet)
    await rollbacked_db_session.commit()
    await rollbacked_db_session.refresh(new_tweet)

    assert len(new_tweet.content) == 280


@pytest.mark.anyio
async def test_posted_at_default(rollbacked_db_session):
...
@pytest.mark.anyio
async def test_user_id_not_null(rollbacked_db_session):
...
async def test_user_id_foreign_key(rollbacked_db_session, new_tweet):
...
@pytest.mark.anyio
async def test_cascade_delete_tweets(rollbacked_db_session):
    ...
    async with rollbacked_db_session.begin():
        rollbacked_db_session.add_all(tweets)

        tweet_qs = await rollbacked_db_session.execute(
            select(models.Tweet).where(models.Tweet.user_id == user_id)
        )
        assert len(tweet_qs.scalars().all()) == len(tweets)

        user_qs = await rollbacked_db_session.execute(
            select(models.User).where(models.User.id == user_id)
        )
        await rollbacked_db_session.delete(user_qs.scalars().one())
