...
async def test_add_user(rollbacked_db_session, new_user):
    ...
    rollbacked_db_session.add(new_user)
...
@pytest.mark.anyio
async def test_nickname_not_null(rollbacked_db_session):
...
@pytest.mark.anyio
async def test_nickname_unique(rollbacked_db_session):
...
async def test_nickname_length(rollbacked_db_session, new_user):
...
async def test_first_name_length(rollbacked_db_session, new_user):
...
async def test_last_name_length(rollbacked_db_session, new_user):
...
@pytest.mark.anyio
async def test_api_key_not_null(rollbacked_db_session):
...
@pytest.mark.anyio
async def test_api_key_unique(rollbacked_db_session):
...
@pytest.mark.parametrize(
    ...
    [
        # короткий api_key
        *generate_test_models(models.User, range(30), nickname="test1", api_key=lambda count: "a" * count),
        # длинный api_key
        *generate_test_models(models.User, range(257, 263), nickname="test1", api_key=lambda count: "a" * count),
    ],
)
async def test_api_key_length(rollbacked_db_session, new_user):
